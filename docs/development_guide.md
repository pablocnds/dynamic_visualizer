...
- **Visualization:** `PlotRenderer` draws single or multiple specs on PyQtGraph widgets, including line/scatter/stick 2-D series plus 1-D colormap strips, event lines, and range bands. Stick styles can label selected peaks by top-N rank and/or an absolute or maximum-relative Y threshold. `TableRenderer` handles tabular views with configurable palette/range rules.
...
- **State persistence:** `StateManager` stores a `current_session` snapshot plus recent-session history. Session snapshots persist live when the user changes source folders/files, switches cards, or updates card variables. Recent-session identity is keyed by the opened source workspace (for example a cards directory or explicitly opened card file), while the active card/data selection and card variables update in place inside that session. Startup restores the current session snapshot, and File > Open Previous Session reopens remembered snapshots. Entries with removed paths or empty data/card folders are pruned automatically; if a saved card file disappears but its directory is still valid, restore falls back to that directory.
...
## Data Contracts
- JSON series: must match `src/visualizer/schema/data_payload.schema.json` (`data.x_axis` required; optional `data.y_axis` must match length when present); non-numeric Y is rejected; labels are optional. `data.x_axis` may be numeric or categorical, and 2-D line/scatter/stick plots map categorical X values onto labeled positions. Eventline plots ignore `data.y_axis`.
